...
class ReparamLargeKernelConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if small_kernel_merged:
            ...
            if small_kernel is not None:
                assert small_kernel <= kernel_size, '重参数化的核大小不能大于大核！'
    ...
    def PaddingTwoEdge1d(self, x, pad_length_left, pad_length_right, pad_values=0):
        ...
        if pad_values == 0:
            pad_left = torch.zeros(D_out, D_in, pad_length_left, device=x.device)
            pad_right = torch.zeros(D_out, D_in, pad_length_right, device=x.device)
        else:
            pad_left = torch.ones(D_out, D_in, pad_length_left, device=x.device) * pad_values
            pad_right = torch.ones(D_out, D_in, pad_length_right, device=x.device) * pad_values
...
class ModernTCNCore(nn.Module):
    def __init__(self, patch_size, patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
                 ...
                 subtract_last=False, seq_len=512, c_in=7, individual=False, target_window=96):
        super(ModernTCNCore, self).__init__()
        ...
        self.head_detection1 = nn.Linear(d_model, self.patch_size)
...
class ModernTCN(nn.Module):
    def __init__(self, seq_len, n_features, patch_size=12, patch_stride=6, stem_ratio=2, downsample_ratio=6, ffn_ratio=1,
                 num_blocks=[1], large_size=[31], small_size=[3], dims=[6], dw_dims=[6],
                 small_kernel_merged=False, backbone_dropout=0.1, head_dropout=0.1,
        ...
        self.model = ModernTCNCore(
            ...
            seq_len=seq_len,
            c_in=n_features,  # 输入通道数
            individual=individual,
            target_window=seq_len  # 输出序列长度与输入相同
        )

    def forward(self, x):
        """
        输入: (batch_size, seq_len, n_features)
        输出: (batch_size, seq_len, n_features)
        """
        x = x.permute(0, 2, 1)
        x = self.model(x)
        return x
